    ...
    fun runAgent(instruction: String, apiKey: String, context: Context, onAgentStopped: (() -> Unit)? = null, onOutput: ((String) -> Unit)? = null) {
        ...
        agentJob = CoroutineScope(Dispatchers.Default).launch {
            ...
            try {
                ...
                // Retrieve configured Base URL and Model Name
                val baseUrl = SharedPrefsUtils.getBaseUrl(context)
                val modelName = SharedPrefsUtils.getModelName(context)
...
Focus on completing the user's request efficiently and accurately.""")
                ...
                while (step < MAX_STEPS && !stopping) {
                    ...
                    LogManager.log(TAG, "Step $step: Calling LLM ($modelName) with tools...")
                    ...
                    try {
                        ...
                        // Pass configured base URL and Model Name
                        val llm = LLMClient(apiKey, baseUrl, modelName)
